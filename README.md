...
- 菜品数据里的钠照算照存（`Nutrients.sodium`），`npm run validate` 也仍然守着标定：家常菜按「一人份」调味，盐当量不超过 2 g（钠 800 mg），单份超 900 mg 会 warning；高汤按家庭无盐高汤计（80 mg/100 g）。外卖与便利店按实际口味不设限。
- **但钠不参与任何判定与展示**（2026-09-20 决定）。原因是两道家常菜加一碗汤就接近 2000 mg 的指南上限，中国居民日均盐摄入约 9~10 g、是指南的两倍——按钠打分等于把每道菜都标红，标了等于没标。
- 随之删掉的还有「少盐做法」开关：它唯一的作用是把调味钠减半，钠不再是指标之后就没有意义了。**「少油做法」是另一回事，仍然在用**（`canLowOil / oilFat / dishNutrientsFor`，`LogEntry` 与 `PlanItem` 的 `lowOil`）。
- 高血压模式下也不逐日算钠，改为避开腌腊、优先清淡做法，加一句静态的少盐提醒（`conditions.ts` 的 `htn_salt_habit`）。

## 推荐的全天收敛

单餐只按热量与蛋白收敛，三餐叠起来脂肪会跑偏（减脂档案实测：脂肪超目标 40%）。`planner.ts` 的 `balanceDay` 在三餐选完后再做一轮全天收敛：

1. 蛋白与热量先到位（加瘦蛋白份量、调主食）。
2. 两轮脂肪：超 10% 先把最油的菜换成同角色更清淡的菜，仍超就整体按少油做法计。蛋白菜换菜必须保住 80% 蛋白。
3. 仍超就压份量，先压非蛋白菜。
4. 蛋白不低于 85%，脂肪有余量再补到 90%；最后只用主食把热量调进 −5%~+5%。
5. 糖尿病模式复核单餐碳水上限。
6. 压不进去的差距如实写进说明。

选菜权重还带一条静态的脂肪供能比惩罚。实测（女 / 1994 / 163 cm / 58 kg / 体脂 26% / 轻活动 / 减脂 / 中式三餐，20 个种子）：目标 1460 千卡、蛋白 94 g、脂肪 45 g，得到均值 1440 / 102 / 47，热量偏差 −5.0%~+6.5%。同一组里钠均值 3896 mg——这就是中式家常菜不做任何假设时的真实水平，也是上面那节不把钠当指标的理由。

## 自定义食物

录餐面板搜不到时可自定义，两种录法：

- **按食材搭配**（默认）：搜食材、填一人份克重，营养、蔬菜/水果/奶类份数、少油做法都和菜品库同一套逻辑推导；保存为自建菜（id 以 `custom_` 开头，存 `AppState.customDishes`），以后搜索、推荐、分析一视同仁。四季豆炒肉这类家常菜用这个。
...
一颗蛋（`src/ui/Creature.tsx` 的 `EggView`）：还没记过东西时在「今日」页占位。第一次记录（三餐或喝水，任一入口）孵化成一只生物——一次随机定型六个外观特征（体型/主色/花纹/眼睛/嘴巴/配饰）加一个性格（元气/温柔/傲娇/高冷，`src/core/creature.ts` 的 `PERSONALITIES`），不做多阶段孵化进度；之后每记一笔，随机挑一个外观特征槽换成不同的值，肉眼看得出变了样但不叠加进度条；性格孵化时定型、终生不变，回炉重造才会重新随机。逻辑在 `src/core/creature.ts`（`hatch` / `mutate` / `retire`，纯函数、可单测），落地由 `App.tsx` 的 `bumpCreature`（挂在跟 `noticeAfterLog` 相同的记录入口上，以及 `setWater` 里真的加水那一支）触发。「我的」页「健康小管家」卡可以「回炉重造」：当前这只存进 `AppState.creatureHistory`（历史里看得到最终形象、性格与异变次数），当前生物变回一颗新蛋。全部形象都是内联 SVG 现画的，不依赖外部图片资源。老数据没有 `personality` 字段的（这个属性是后加的），`storage.ts` 的 `normalizeState` 会按生物 id 哈希稳定补一个，不会每次刷新都变。

形象定稿走的是「有机趣味」方向：先用一个独立的 HTML 对比页给了三个视觉方向的实际渲染（圆润软萌 / 几何简线 / 有机趣味），每个方向配几个随机特征组合的小样，用户挑了「有机趣味」——保留原来那种不规则生物感，但轮廓收得更利落，配色仍是应用一贯的扁平无阴影渐变。定稿后把 `BODY_PATH` 的四个体型（圆/蛋/团/水滴）都按这个方向重画成一个手绘家族，另加了第 7 种配饰 `sprout`（呼应营养主题的一对小嫩芽，固定草绿色不随体色变，见 `src/core/creature.ts` 的 `EXTRAS` 与 `Creature.tsx` 的 `Extra`）。花纹/眼睛/嘴巴的渲染函数本身不用换，对比页里就是拿现成的这几套直接叠上去看效果，站得住。

孵化后小管家会在「今日」页冒对话气泡（`src/ui/Creature.tsx` 的 `SpeechBubble`），只在看「今天」时才念叨；不是随机瞎聊，而是复用现成的信号按优先级选一句最要紧的：蛋刚孵化打招呼（仅这一次，下次记录触发异变就恢复正常）→ 喝水明显落后 → 到点了但那一餐没记（按早中晚顺序）→ 下午了今天还没吃水果 → 高血压模式下钠已超上限 → 近 7 天分析里归为「饮食习惯」类的提醒（`src/core/analysis.ts` 新导出的 `isHabitFinding`，`Analysis.tsx` 也改用同一个函数分类，行为不变）→ `budgetFocus` 里最靠前的超额 → 最靠前的缺口 → 都没有就说一句轻松话（按日期取一句，同一天稳定不换）。判断逻辑在 `src/core/creatureTalk.ts` 的纯函数 `creatureLine`，不重新发明喝水节奏或营养缺口的算法；看别的日期只给轻松话，不提醒任何进度。同一句话怎么说由小管家的性格决定：`creatureTalk.ts` 的 `TONE` 表给四种性格各准备一套文案（内容——该提醒什么——完全一致，只是措辞冷热不同），优先级链本身不受性格影响。

小管家还有待机动画和表情：`Creature.tsx` 内联 `<style>` 关键帧（不改共享的 `styles.css`，避免和当时正在改这个文件的另一会话冲突）让生物轻轻呼吸（`translateY` 起伏）+ 定期眨眼（眼睛整体 `scaleY` 瞬间压扁再弹回，五种眼型都适用），蛋孵化前也会轻轻摇晃；每只生物按长相哈希出一个动画延迟种子，历史图鉴里一排不会齐刷刷同步。表情是「心情」而不是改变 trait 决定的眼嘴造型——`creatureTalk.ts` 把 `creatureLine` 内部的优先级链抽成共享的 `pick()`，新导出 `creatureMood()` 返回同一次判断算出的心情档位（`excited`蛋刚孵化 / `happy`一切正常或轻松话 / `neutral`该做点什么但不算糟 / `concerned`超标或习惯提醒），只在头顶叠一个小星芒（开心/兴奋）或汗滴（担心）图标做点缀，不动脸本身。只有「今日」页当前生物会显示心情点缀（有完整的今日信号可用）；「我的」页的当前预览与历史图鉴只有待机动画、心情固定中性。全部动画都遵守 `prefers-reduced-motion: reduce`。

异变（长相变了但组件没卸载重挂载）不会瞬间跳变：`CreatureView` 会先冒一团烟盖住旧长相（几个软圆叠出云团感，一次性播放约 0.7 秒），盖住的那一刻才把实际画出来的 `displayed` 状态换成新 traits，散开时正好露出新样子。这个「冒烟换脸」跟待机动画一样是内联 `<style>` 关键帧，不进 `styles.css`；每次异变通过 `poofKey` 强制烟雾节点重新挂载，即使连续快速异变也不会用到上一次没播完的动画。不依赖 `onAnimationEnd` 单独收尾——实测这个事件在部分环境下不总是可靠触发，会导致烟雾播完了但节点没卸载、一直挡在最上层——所以额外加了一个跟动画总时长对齐的 `setTimeout` 兜底强制收起，双保险。

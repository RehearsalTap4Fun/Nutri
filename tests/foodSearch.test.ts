...
describe('地方小吃与正餐连锁（2026-09-20 批次）', () => {
  it('候选来自公开小吃盘点，逐条对过差集后补的，都要搜得到', () => {
    const NAMES = ['驴肉火烧', '鸡蛋灌饼', '锅盔', '土豆粉', '花甲粉', '臭豆腐', '狼牙土豆', '烤面筋',
      '炸串', '钵钵鸡', '串串香', '卤煮', '炒肝', '糖葫芦', '糖油粑粑', '驴打滚', '麻球', '烧仙草']
    for (const q of NAMES) expect(top(q), q).not.toBeNull()
  })

...
- **推荐**：按当天剩余预算排三餐。每餐可单独换一换（各自的种子，不动别的餐），可以整餐「照这个吃」记为已吃，单道菜可以叉掉进不推荐名单。每道菜给出份量、理由标签、做法与蛋白。已吃的餐次改为列出实际吃了什么。另有采购清单和排餐原因折叠。
- **分析**：近 7 天热量柱状图、近 4 周达标日历（连续天数与总天数）、「日均 vs 目标」的偏差柱（围绕目标线，带合适区间带，按结论上色）、三大营养素供能比与参考范围折叠、按「数值」与「习惯」分组的结论、体重折线图与记录、以及按体重反推的 TDEE 校准开关。
- **健康小管家**：今日页顶部。记下第一笔孵化，之后每记一笔推进一阶，长出的部件、达成的称号和成长进度都在卡片里；孵化、异变、解锁称号会在记录成功的提示里说出来。
- **数据与目标依据**：在「我的」页。目标依据逐项列出规则与出处（`@core/sources`）。导出走剪贴板——小程序没有文件下载，这是唯一可行的出口；导入是粘贴回来。导出内容不含 API key 与同步码。
...
**待机呼吸已经有了**：以脚为原点的 3.4 秒缩放，参数与网页版 `pixelcat-breathe` 一致，每只猫按 spec 的哈希错开相位，同屏几只不会一起起伏。`prefers-reduced-motion` 下关掉。

动画作用在 `Image` 上而不是 canvas 上，这一点是有意的——原生层的画布做 CSS 动画在真机上不可靠。**以后要做逐帧动画也走这条路**：预先导出几帧再换 `src`，不必把活画布放回版面。

网页版还有而这边没有的两样，都靠 SVG 覆盖层实现，搬过来要先出位图：**心情点缀**（`MoodAccent`）和**异变时的冒烟换脸**（`SmokePoof`）。
